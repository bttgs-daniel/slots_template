...
namespace Mkey
{
    public class SlotControls : MonoBehaviour
    {
        ...
        private IEnumerator Start()
        {
            ...
            if (InfoText) infoCoinsTween = new TweenLongValue(InfoText.gameObject, 0, 0.5f, 2, true, (w) => { if (this) SetTextString(InfoText, (w > 0) ? w.ToString(coinsFormat) : "0"); });
        ...
        private void RefreshSpins()
        {
            ...
            if (FreeSpinText) FreeSpinText.gameObject.SetActive(FreeSpins > 0);
            if (SpinText) SpinText.gameObject.SetActive(FreeSpins <= 0);
